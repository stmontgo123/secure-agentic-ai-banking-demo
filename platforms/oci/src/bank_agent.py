"""Secure agentic banking demo.

The agent gathers deterministic evidence, retrieves only role-authorized policy,
asks a local LLM to synthesize a grounded recommendation, and creates a PENDING
action request. It never approves or executes the consequential decision.
"""
...
def build_prompt(transaction, account, fraud_events, policies) -> str:
    policy_context = "\n\n".join(
        f"{p[0]} - {p[1]}:\n{clob_text(p[2])}" for p in policies
    )
...
def call_llm(prompt: str) -> str:
    ...
    response = requests.post(
        f"{base}/api/generate",
        json={
            "model": os.environ.get("GEN_MODEL", "qwen2.5:3b"),
            "prompt": prompt,
            "stream": False,
...
def investigate(transaction_id: int) -> int:
    ...
    print("[1] Transaction evidence retrieved")
    ...
    print(f"[4] Authorized policy retrieval completed ({len(policies)} chunk(s))")
    ...
    write_audit(
        ...
        "LLM_RESPONSE",
        "GENERATED",
        {"model": os.environ.get("GEN_MODEL", "qwen2.5:3b"), "response": recommendation},
    )
    print("[6] AI response generated\n")
